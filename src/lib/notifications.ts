...
export async function deliverNotifications(db: AppDb, form: Form, submission: Submission): Promise<DeliveryResult[]> {
  ...
  if (form.notifyEmail && form.emailTo) {
    // Check if custom SMTP/Gmail configuration is present in environment or settings
    const isGmailSmtp = form.emailTo.includes("@gmail.com");
    ...
    if (env.RESEND_API_KEY && env.RESEND_FROM) {
      ...
      // Fallback: Dispatch using direct SMTP API Gateway or Gmail HTTP Web service
      try {
        const mailResponse = await fetch("https://api.mailchannels.net/tx/v1/send", {
          ...
          body: JSON.stringify({
            ...
            from: { email: "notifications@formforge.dev", name: "FormForge Notifications" },
            subject: `New ${form.name} Form Submission`,
            content: [{
              type: "text/plain",
              value: `FormForge new submission alert!\n\nPayload:\n${JSON.stringify(payload, null, 2)}`
            }]
          })
        });
        results.push(
          mailResponse.ok
            ? { channel: "email", status: "sent" }
            : { channel: "email", status: "failed", error: await mailResponse.text() }
        );
      } catch (err) {
        results.push({ channel: "email", status: "failed", error: err instanceof Error ? err.message : "SMTP fallback failure" });
      }
    } else {
      results.push({ channel: "email", status: "skipped", error: "Missing Resend API configurations keys." });
    }
  } else {
